...
def compute_pck(pred, target, threshold=0.05):
    # pred: 预测的关节位置，形状为 [batch_size, num_joints, 3]
    # target: 真实的关节位置，形状为 [batch_size, num_joints, 3]
    # threshold: 容许误差阈值，通常以尺度或单位来衡量

    # 计算每个关节的欧氏距离
    dist = torch.norm(pred - target, p=2, dim=-1)  # 计算欧氏距离

    # 判断预测是否在阈值范围内
    correct = dist < threshold  # 判断哪些关节在阈值内
    pck = correct.float().mean()  # 计算PCK的平均值
    return pck.item()
#auc

def compute_auc_without_sklearn(pred, target, thresholds=None):
    if thresholds is None:
        thresholds = np.linspace(0.01, 0.99, 100)  # 设置不同的阈值，通常从 0.01 到 0.99
    
    pck_scores = []
    for threshold in thresholds:
        pck = compute_pck(pred, target, threshold)  # 使用前面定义的 compute_pck
        pck_scores.append(pck)

    # 使用梯形法则计算 AUC
    auc_score = np.trapz(pck_scores, thresholds)
    return auc_score
...
def evaluate(model, test_loader, n_frames):
    ...
    for data in tqdm(test_loader, 0):
        ...
        #pck
        pck_score = compute_pck(pred_out, out_target)
        pck_scores.append(pck_score)
        #auc
        auc_score = compute_auc_without_sklearn(pred_out, out_target)  # 使用自定义的AUC计算方法
    ...
    print(f'Average PCK: {np.mean(pck_scores):.2f}')
